...
cssid: test
excerpt: Jupyter Notebook, cram it in a Docker then throw a stack of  Ruby NLP Gems on top of that and you got yourself a thing.
layout: page
toc: true
tags:
  - docker
  ...
  - nlp
  - llm
introduction: Greetings! In this notebook entry, we will walk through how to use Docker images to create a local Jupyter notebook environment with Ruby support. Setting up the proper Docker container requires choosing the right public images and making a few configuration tweaks. Though it may sound complicated, the process is actually quite straightforward. We will select official images for Jupyter and Ruby, join them together with a Dockerfile, and configure the environment to meet our needs. No fancy hacks required! By the end, we will have a simple yet effective Docker container for developing with Ruby in Jupyter. There are many benefits to this approach over managing Python environments locally. Creating the container only takes a few steps. Let's dive in and see just how easy it is!
---

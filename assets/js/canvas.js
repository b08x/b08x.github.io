...
// Enhanced zoom/pan configuration
const ZOOM_SPEED = 0.15; // Increased for better sensitivity
const ZOOM_SMOOTH_FACTOR = 0.12; // Lower = smoother (0.05-0.2 range)
const PAN_SMOOTH_FACTOR = 0.15; // Smooth panning momentum
const minScale = 0.35;
const maxScale = 1.25;
...
// Global API for React components
window.canvasAPI = {
  getScale: () => scale,
  ...
  zoomIn: () => {
    ...
    zoomToPoint(scale + ZOOM_SPEED, centerX, centerY);
    dispatchCanvasUpdate();
  },
  ...
  updateCanvas: (data) => {
    // Rebuild canvas nodes safely (no innerHTML for security)
    const container = document.getElementById('canvas-nodes');
    container.replaceChildren(); // Safe alternative to innerHTML = ''

    // Reconstruct nodes from data
    data.nodes?.forEach(nodeData => {
      ...
      nodeName.textContent = nodeData.id;
      ...
      } else if (nodeData.type === 'link' && nodeData.url) {
        const link = document.createElement('a');
        link.className = 'node-link-content';
        link.href = nodeData.url;
        link.textContent = nodeData.text || nodeData.url;
...
function adjustCanvasToViewport() {
  ...
  // Initialize target values for smooth animation
  targetScale = scale;
  targetPanX = panOffsetX;
  targetPanY = panOffsetY;
...
// Enhanced cursor-centered zoom with smooth easing
window.addEventListener('wheel', (e) => {
    // Support both Ctrl+wheel and regular wheel for zoom
    if (e.ctrlKey || e.metaKey) {
        e.preventDefault();

        // Dynamic zoom speed based on wheel delta (more sensitive)
        const zoomFactor = -e.deltaY * 0.001; // Smoother increments
...
// Legacy button event listeners removed - now handled by React components
// (CanvasControls, OutputPanel, CanvasExporter)
...
function updateCanvasData() {
  ...
  // Legacy DOM output removed - now handled by React OutputPanel component
  // Just dispatch event for React components to consume
  dispatchCanvasUpdate();
}
...
function smoothAnimate() {
  // Ease towards target values
  const scaleDiff = targetScale - scale;
  const panXDiff = targetPanX - panOffsetX;
  const panYDiff = targetPanY - panOffsetY;

  // Apply easing (lerp interpolation)
  scale += scaleDiff * ZOOM_SMOOTH_FACTOR;
  panOffsetX += panXDiff * PAN_SMOOTH_FACTOR;
  panOffsetY += panYDiff * PAN_SMOOTH_FACTOR;
  ...
  if (Math.abs(scaleDiff) > threshold || Math.abs(panXDiff) > threshold || Math.abs(panYDiff) > threshold) {
...
// Zoom towards a specific point (cursor-centered zoom)
function zoomToPoint(newScale, clientX, clientY) {
  ...
  const worldX = (mouseX - panOffsetX) / scale;
  const worldY = (mouseY - panOffsetY) / scale;

  // Set new target scale
  targetScale = newScale;

  // Calculate new pan offsets to keep world coordinates under mouse

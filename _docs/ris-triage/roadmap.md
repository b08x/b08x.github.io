...
permalink: /docs/ris-triage/roadmap/
doc_set: ris-triage
...
next:
  ...
  url: /docs/ris-triage/order-lifecycle/
---
...
**ADR backfill.** Eight architectural decisions are recorded in the [index](/docs/ris-triage/decisions/); individual records are still being written up. The decisions were made in design, not retrofitted — but only [ADR-0008](/docs/ris-triage/adr-0008-separate-forensics-plugin/) exists as a full record so far.
...
The reasoning is in [ADR-0008](/docs/ris-triage/adr-0008-separate-forensics-plugin/); parked work is tracked in [FORENSICS-BACKLOG.md](https://github.com/b08x/ris_triage/blob/main/docs/FORENSICS-BACKLOG.md).
...
- **The `systems` schema is single-site.** A practice supporting many facilities across several PACS platforms cannot be represented cleanly, and `/setup` will ask you how to model it rather than deciding. See [site profile setup](/docs/ris-triage/site-profile/).
...
Developed in collaboration with Claude, and that process is published rather than hidden. Decision records capture why it is built this way, including rejected alternatives. [Guardrails](/docs/ris-triage/guardrails/) documents the specific failure modes the collaboration is checked against — design and drafting are co-developed; behaviour rules and published claims are validated by a human before release.
